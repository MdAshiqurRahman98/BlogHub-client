...
const BlogDetails = () => {
    ...
    const handleComment = event => {
        ...
        const newComment = { blogId: _id, title, category, userName, userPhoto, comment, timestamp: getCurrentTimestamp() };
    ...
    const { data: comments = [], refetch } = useQuery({
        queryKey: ['comments', user?.email, _id],
        queryFn: async () => {
            const res = await axiosSecure.get(`/all-comments?email=${user?.email}&_id=${_id}`);
            return res.data;
        }
    })
    ...
    return (
        <>
            <Helmet>
                <title>Blog Details | BlogHub</title>
            </Helmet>
            <div className="my-11">
                ...
                <Comments key={_id} comments={comments}></Comments>
            </div>
        </>
    );
};
